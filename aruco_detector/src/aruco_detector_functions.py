...
def aruco_detector(img, aruco_dict, markerLength, markerID, cam_mtx, dist):
...
def axis_converter(rot_quaternion, dest_axis='z', degree=90):
    if dest_axis == 'z':
        # X ---> Z axis: rotate -90° along the Y axis
        z_rot_quaternion = Quaternion(axis=[0.0, 1.0, 0.0], degrees=degree)
        rot_quaternion *= z_rot_quaternion
    elif dest_axis == 'y':
        # X ---> Y axis: rotate +90° along the Z axis
        y_rot_quaternion = Quaternion(axis=[0.0, 0.0, 1.0], degrees=degree)
        rot_quaternion *= y_rot_quaternion
    elif dest_axis == 'x':
        x_rot_quaternion = Quaternion(axis=[1.0, 0.0, 0.0], degrees=degree)
        rot_quaternion *= x_rot_quaternion
    else:
...
#---------- function: pose_msg_sender ----------
#   purpose: Convert the obtained position information to "geometry_msgs.TransformStamped" type information.
#   parameters:
#   ~rot_vec: Rotation vector
#   ~trs_vec: Translation vector
#   ~dest_axis: destinate rotation (default: Z axis, perpendicular to the camera).
def pose2msg_converter(rot_vec, trs_vec, dest_axis='y'):
    ...
    #rot_quaternion = axis_converter(rot_quaternion, 'z')
    #rot_quaternion = axis_converter(rot_quaternion, 'y', 90)
    #rot_quaternion = axis_converter(rot_quaternion, 'x', 270)
    
    trans.transform.translation.x = trs_vec[0]
    trans.transform.translation.y = trs_vec[1]
    trans.transform.translation.z = trs_vec[2]
    trans.transform.rotation.w = rot_quaternion[0]
    trans.transform.rotation.x = rot_quaternion[1]
    trans.transform.rotation.y = rot_quaternion[2]
    trans.transform.rotation.z = rot_quaternion[3]
